---
title             : "Analytic reproducibility in articles receiving open data badges at Psychological Science: An observational assessment"
...
load(here('data', 'processed', 'data_valuesLevel.RData')) # load data
load(here('data', 'processed', 'data_articlesLevel.RData')) # load data
load(here('data', 'processed', 'data_kidwell.RData')) # load data
```
...
```
Prior to author involvement, our reproducibility checks indicated that `r reproBefore %>% filter(reproOutcome == F) %>% pull(n)` out of the `r totalArticles` (`r reproBefore %>% filter(reproOutcome == F) %>% pull(percent)`) articles contained at least one major numerical discrepancy. After requesting input from original authors, several issues were resolved through the provision of additional information[^authorInfo] leaving `r reproAfter %>% filter(reproOutcome == F) %>% pull(n)` out of the `r totalArticles` (`r reproAfter %>% filter(reproOutcome == F) %>% pull(percent)`) articles containing at least one major numerical discrepancy. Even after author contact, we could not complete `r insuffInfoErrors` (`r insuffInfoErrors_percent`%, `r insuffInfoErrors_CIs`) reproducibility checks because insufficient information was available about aspects of the original analyses. 

Ultimately, target values were fully reproducible for `r articleLevelSummary %>% filter(finalOutcome == 'Reproducible\nwithout author\ninvolvement') %>% pull(fullReport)` articles without requiring author involvement; fully reproducible for `r articleLevelSummary %>% filter(finalOutcome == 'Reproducible\nwith author\ninvolvement') %>% pull(fullReport)` articles only with author involvement; not fully reproducible for `r articleLevelSummary %>% filter(finalOutcome == 'Not reproducible\nwith author\ninvolvement') %>% pull(fullReport)` articles despite author involvement; and not fully reproducible for `r articleLevelSummary %>% filter(finalOutcome == 'Not reproducible\nwithout author\ninvolvement') %>% pull(fullReport)` articles with no substantive response from authors. Availability of original analysis scripts appeared to provide some modest benefits in terms of reproducibility outcomes and time expenditure (see Supplementary Information \@ref(sup_results)).
...
```{r scatterPlot, fig.cap="Scatterplot showing reported p-values as a function of reanalysis p-values, classified by reproducibility outcome. Axes are on a log scale. For display purposes, 41 values below 0.001 are not shown.", fig.path="figs/"}
